...
public class Locacoes {
    ...
    public void baixaLocacao(int num, Rep_pessoas pessoas, Rep_produtos produtos){
        Locacoes locacao = locacoes.get(num);
        Produto produto = produtos.getProduto(locacao.getCodigo());
        Pessoa cliente = pessoas.getPessoa(locacao.getMatricula());

        double total = 0;
        int dias = calcularDias(locacao.getData_saída(), LocalDate.now());
        int multa_dias = calcularDias(locacao.getData_entrega(), LocalDate.now());
        ...
        if(multa_dias > 0) System.out.println("\n! Dias de atraso: " + multa_dias);
        if (multa > 0) System.out.println(multa_txt);
